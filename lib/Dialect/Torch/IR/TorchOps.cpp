...
static inline torch_upstream::ScalarType materializeQType(torch_upstream::ScalarType t) {
...
static LogicalResult commuteQuantizedConvolution(QuantizationOp op, PatternRewriter &rewriter) {
  auto result = op.getResult();
  if (!result.hasOneUse())
    return rewriter.notifyMatchFailure(
        op, "quantize op has multiple uses");
...
void AtenQuantizePerTensorOp::getCanonicalizationPatterns(RewritePatternSet &patterns,
                                                MLIRContext *context) {
  patterns.add(+[](AtenQuantizePerTensorOp op, PatternRewriter &rewriter) {
    ...
    auto dtypeValue = rewriter.create<Torch::ConstantIntOp>(loc, rewriter.getI64IntegerAttr((int64_t)scalarDtype));
...
void AtenQuantizePerChannelOp::getCanonicalizationPatterns(RewritePatternSet &patterns,
                                                MLIRContext *context) {
